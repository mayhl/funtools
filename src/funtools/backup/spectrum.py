...
from funtools.math.waves import solveDispersion


class _Spectrum(param.Parameterized):

    _pos_def_kwargs = dict(bounds=(0, None))
    ...
    interp_mode = param.Selector(
        default="simpson", objects=["left", "trapezoidal", "simpson"]
    )
    ...
    def _discrete_energy(self):
        ...
        args = (
            self._compute_equal_energy()
            if self.equal_energy
            else self._compute_equal_frequency()
        )
    ...
    def _update_max_wave_info(self):
        self._k_max, self._length_max, self._period_max = self._get_wave_info(
            self.freq_max
        )

    @param.depends("freq_min", watch=True)
    def _update_min_wave_info(self):
        self._k_min, self._length_min, self._period_min = self._get_wave_info(
            self.freq_min
        )

    @param.depends("freq_peak", watch=True)
    def _update_peak_wave_info(self):
        self._k_peak, self._length_peak, self._period_peak = self._get_wave_info(
            self.freq_peak
        )
    ...
    def length_peak(self):
        return self._length_peak
...
def generate_funwave_files(dpath, spec_fname, fname_mask, wave_data, spec):
    n, _ = wave_data.shape
    args = [
        _gen_spec_data_file(i + 1, dpath, fname_mask, spec, *wave_data[i, :])
        for i in range(n)
    ]
